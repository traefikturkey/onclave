...
import { createRemoteHubClient, type RemoteHubClientIdentity } from "../../src/coms-lan/remote-client";
...
describe("coms-lan acceptance coverage", () => {
  it("starts one local hub, reuses it, registers through WSS, and completes a local round trip", async () => {
    const root = await mkdtemp(join(tmpdir(), "coms-lan-acceptance-bootstrap-"));
    tempDirs.push(root);
  ...
  it("lists and sends to a trusted remote hub after public keys are exchanged", async () => {
    ...
    const server = new ComsLanHubRuntime({
...
async function generateSelfSignedTls(): Promise<{ tempDir: string; tls: TlsMaterial }> {
  const dir = await mkdtemp(join(tmpdir(), "coms-lan-acceptance-tls-"));
  execFileSync(
    "openssl",
    [
      "req",
