...
import { loadOrCreateTlsMaterial } from "../../src/coms-lan/tls";
...
describe("loadOrCreateTlsMaterial", () => {
  it("generates and persists TLS material under the coms-lan root", async () => {
    const root = await mkdtemp(join(tmpdir(), "coms-lan-tls-"));
    tempDirs.push(root);
