...
import { findStaticPeer, loadComsLanConfig, parseComsLanConfig, writeComsLanConfig } from "../../src/coms-lan/config";
...
describe("coms-lan config", () => {
  it("returns an empty default config when config.json is missing", async () => {
    const root = await mkdtemp(join(tmpdir(), "coms-lan-config-"));
    tempDirs.push(root);
    await expect(loadComsLanConfig(getComsLanPaths(root))).resolves.toEqual({ version: 1, staticPeers: [] });
  });
  ...
  it("writes validated config", async () => {
    ...
    tempDirs.push(root);
    const paths = getComsLanPaths(root);
    await writeComsLanConfig(paths, {
      version: 1,
      staticPeers: [{ nodeId: "node_a", hubInstanceId: "hub_a", endpoint: "wss://host:1111/v1/hub" }],
    });

    await expect(loadComsLanConfig(paths)).resolves.toEqual({
      version: 1,
      staticPeers: [{ nodeId: "node_a", hubInstanceId: "hub_a", endpoint: "wss://host:1111/v1/hub" }],
    });
  });
