...
import { loadIdentityPrivateKeyHex, loadOrCreateIdentity } from "../../src/coms-lan/identity";
...
describe("loadIdentityPrivateKeyHex", () => {
  it("loads the app-specific private signing key from the coms-lan state root", async () => {
    const root = await mkdtemp(join(tmpdir(), "coms-lan-identity-key-"));
    tempDirs.push(root);
