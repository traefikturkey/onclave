...
2. In either session run `/onclave` and the `onclave_agents` tool: both
   agents should list with `alive=true`, and the footer should show the local
   client name in green with the current `Peers` count.
...
2. The receiver must display `verified operator delegation`, the grant id,
   actions, expiry, and scope, then execute the bounded request without an
   additional Onclave prompt or per-sender configuration.
3. Alter the body, audience, project, or expiry. The receiver must reject the
   request and produce no turn.
4. Send an ordinary `onclave_send` request. It must retain the original
