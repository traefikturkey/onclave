...
export type BwsRunner = (
  command: string,
  args: string[],
  options: { env: NodeJS.ProcessEnv; timeout: number; maxBuffer: number; signal?: AbortSignal }
) => Promise<{ stdout: string }>;

/** A sanitized marker for failures executing the BWS command itself. */
export class BwsCommandExecutionError extends Error {
  readonly retryable = true;

  constructor() {
    super("Onclave could not read its Bitwarden Secrets Manager project");
    this.name = "BwsCommandExecutionError";
  }
}

export function isRetryableBwsCommandError(error: unknown): error is BwsCommandExecutionError {
  return error instanceof BwsCommandExecutionError && error.retryable;
}
...
async function loadBwsSecrets(
  ...
  try {
    ...
    // A timeout generated by bootstrap recovery is retryable, while an abort
    // from session shutdown/replacement remains cancellation. Never expose the
    // subprocess error or its output to callers.
    if (signal?.aborted) {
      const reason = signal.reason;
      if (reason instanceof DOMException && reason.name === "TimeoutError") throw new BwsCommandExecutionError();
      throw reason ?? new DOMException("The operation was aborted", "AbortError");
    }
    throw new BwsCommandExecutionError();
  }
...
export async function loadApiBaseFromBws(
  ...
  const secrets = await loadBwsSecrets(environment, runner, signal);
