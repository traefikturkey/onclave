...
import { formatAuthorizedKeyLine } from "../src/lib/trust";
import type { DiscoveryUdpSocket, UdpRemoteInfo } from "../src/lib/discovery";
import { signHandshakePayload, type HandshakePayload, type ServerHelloFrame } from "../src/lib/handshake";
import type { RemoteHubClient } from "../src/lib/remote-client";
...
import type { ClientAuthFrame } from "../src/lib/transport";
import { sendAuthenticatedWssFrames, type TlsMaterial } from "../src/lib/wss-transport";
...
async function generateSelfSignedTls(): Promise<{ tempDir: string; tls: TlsMaterial }> {
  const dir = await mkdtemp(join(tmpdir(), "onclave-runtime-"));
  execFileSync(
    "openssl",
    [
      "req",
