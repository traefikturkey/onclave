...
export type OnclaveHubRuntimeOptions = {
  nodeId: string;
...
export class OnclaveHubRuntime {
  ...
  hubState(): HubState {
    if (!this.state) throw new Error("onclave hub runtime is not started");
    return this.state;
  }

  wssUrl(): string {
    if (!this.wssServer) throw new Error("onclave hub runtime is not started");
